...
const Register: React.FC<RegisterProps> = ({ onRegisterSuccess, navigateToLogin, isLoading, setIsLoading }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!email.trim()) {
      ...
      currentError = 'សូមបំពេញគ្រប់ช่องទាំងអស់។ (Please fill in all fields.)';
      if(!focused) confirmPasswordInputRef.current?.focus();
      focused = true;
      ...
      currentError = 'ពាក្យសម្ងាត់ និងការបញ្ជាក់ពាក្យសម្ងាត់មិនត្រូវគ្នាទេ។ (Passwords and confirm password do not match.)';
      if(!focused) confirmPasswordInputRef.current?.focus();
      focused = true;
    }
    ...
    setIsLoading(true);
    // Mock registration success after a short delay
    setTimeout(() => {
      onRegisterSuccess();
    }, 1000);
  };
  
  const isFieldInvalid = (field: 'email' | 'password' | 'confirmPassword'): boolean => {
    ...
    switch (field) {
      case 'email':
        ...
        break;
      case 'password':
        // Check for errors specifically about the password field, not confirmation
        if ((lowerError.includes('password') || lowerError.includes('ពាក្យសម្ងាត់')) && 
            !(lowerError.includes('confirm') || lowerError.includes('បញ្ជាក់') || lowerError.includes('match') || lowerError.includes('មិនត្រូវគ្នា'))) return true;
        if ((lowerError.includes('fill') || lowerError.includes('បំពេញ')) && !password.trim()) return true;
        break;
      case 'confirmPassword':
        if (lowerError.includes('confirm') || lowerError.includes('បញ្ជាក់') || lowerError.includes('match') || lowerError.includes('មិនត្រូវគ្នា')) return true;
        if ((lowerError.includes('fill') || lowerError.includes('បំពេញ')) && !confirmPassword.trim()) return true;
        break;
    }
    return false;
  };
    ...
    <div className={`flex flex-col items-center justify-center h-full bg-gray-100 dark:bg-gray-900 p-4 ${khmerFont}`}>
      <div className="w-full max-w-md p-8 space-y-6 bg-white dark:bg-gray-800 rounded-xl shadow-xl">
        ...
        <p className="text-sm text-center text-gray-600 dark:text-gray-400">
          ...
          <button
            onClick={!isLoading ? navigateToLogin : undefined}
            disabled={isLoading}
